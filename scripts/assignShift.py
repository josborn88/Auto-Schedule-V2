					...
					if (config.dpop.cell(row = h - shift.long, column = who).value not in shift.restrictions) and ((config.dpop.cell(row=21,column=who).value in knows) or (config.dpop.cell(row=21,column=who).value in knowsFirst)):
						...
						for i in range(config.floorStart, config.floorEnd - 1):
							if config.dpop.cell(row = 58, column = i).fill == shift.backFill:
								...
								break
						if shift.clearance != '':
							config.dpop.cell(row = h + len(shift.name.split()), column = who).value = shift.clearance
	...
	while go == True:
		for h in range(31, 59, shift.long):
			...
			while assigned == False:
				...
				if (config.dpop.cell(row = h, column = who).fill == config.blankFill) and (config.dpop.cell(row = h + shift.long - 1, column = who).fill == config.blankFill):			
					if (config.dpop.cell(row = h - (shift.long - 1), column = who).value not in shift.restrictions) and ((config.dpop.cell(row=21,column=who).value in knows) or (config.dpop.cell(row=21,column=who).value in knowsFirst)):
						config.dpop.cell(row = h, column = who).value = shift.name.split()[0]
						if shift.name.split()[0] != shift.name.split()[-1]:
							config.dpop.cell(row = h + 1, column = who).value = shift.name.split()[1]
						config.dpop.cell(row = h, column = who).fill = shift.backFill
						...
						if shift.name == 'LKS Desk':
							config.dpop.cell(row = h, column = who).font = deskFont
							config.dpop.cell(row = h + 1,column = who).font = deskFont
		...
		while assigned == False:
			...
			if (config.dpop.cell(row = times.get(shift.start), column = who).fill == config.blankFill) and (config.dpop.cell(row = times.get(shift.end), column = who).fill == config.blankFill):			
				if ((config.dpop.cell(row=21,column=who).value in knows) or (config.dpop.cell(row=21,column=who).value in knowsFirst)):
					...
					config.dpop.cell(row = times.get(shift.start), column = who).border = config.borderGoneB
					config.dpop.cell(row = times.get(shift.start) + 1, column = who).border = config.borderGoneT						
					
					if times.get(shift.end) - times.get(shift.start) > 2:
						config.dpop.cell(row = times.get(shift.start) + 1, column = who).border = config.borderGoneTB
						config.dpop.cell(row = times.get(shift.start) + 2, column = who).border = config.borderGoneTB
						config.dpop.cell(row = times.get(shift.start) + 3, column = who).border = config.borderGoneTB
						
					if times.get(shift.end) - times.get(shift.start) > 4:
						config.dpop.cell(row = times.get(shift.start) + 4, column = who).border = config.borderGoneTB
						
					if times.get(shift.end) - times.get(shift.start) > 5:
						config.dpop.cell(row = times.get(shift.start) + 5, column = who).border = config.borderGoneT
					assigned = True
					tried = 0
					go = False
...
def buildClose(shift):
	...
	while go == True:	
		...
		while assigned == False:
			who = random.randint(config.floorStart, config.floorEnd - 1)
			if (config.dpop.cell(row = start, column = who).fill == config.blankFill) and (config.dpop.cell(row = start + 1, column = who).fill == config.blankFill):			
				if ((config.dpop.cell(row = 21,column = who).value in knows) or (config.dpop.cell(row=21,column=who).value in knowsFirst)):
					config.dpop.cell(row = start, column = who).value = time + ' ' + shift.name.split()[0]
					if shift.name.split()[0] != shift.name.split()[-1]:
						config.dpop.cell(row = start + 1, column = who).value = time + ' ' + shift.name.split()[1]
					config.dpop.cell(row = start, column = who).fill = shift.backFill
					config.dpop.cell(row = start, column = who).border = config.borderGoneB
					config.dpop.cell(row = start + 1, column = who).fill = shift.backFillTwo
					assigned = True
					
					for i in range(config.floorStart, config.floorEnd - 1):
						if config.dpop.cell(row = start + 1, column = i).fill == shift.backFillTwo:
							tried = 0
							go = False
							break
...
def buildOpen(shift):
	...
	times = {'9:45a': 30, '10a': 31, '10:15a': 32, '10:30a': 33, '10:45a': 34, '11a': 35, '11:15a': 36, '11:30a': 37, '11:45a': 38, '12p': 39, '12:15p': 40, '12:30p':41, '12:45p':42, '1p':43, '1:15p': 44, '1:30p': 45, '1:45p': 46, '2p': 47, '2:15p': 48, '2:30p': 49, '2:45p': 50, '3p': 51, '3:15p': 52, '3:30p': 53, '3:45p': 54, '4p': 55, '4:15p': 56, '4:30p': 57, '4:45p': 58, '5p': 59, '5:15p': 60, '5:30p': 61}           
	...
	while go == True:	
		...
		while assigned == False:
			who = random.randint(config.floorStart, config.floorEnd - 1)
			if (config.dpop.cell(row = times.get(shift.start), column = who).fill == config.blankFill) and (config.dpop.cell(row = (times.get(shift.start) + (times.get(shift.end) - times.get(shift.start)) -1), column = who).fill == config.blankFill):			
				if ((config.dpop.cell(row = 21,column = who).value in knows) or (config.dpop.cell(row=21,column=who).value in knowsFirst)):
					config.dpop.cell(row = times.get(shift.start), column = who).value = 'OP ' + shift.name.split()[0]
					if shift.name.split()[0] != shift.name.split()[-1]:
						config.dpop.cell(row = times.get(shift.start) + 1, column = who).value = shift.name.split()[1]
					config.dpop.cell(row = times.get(shift.start), column = who).fill = shift.backFill
					config.dpop.cell(row = times.get(shift.start), column = who).border = config.borderGoneB
					for i in range(times.get(shift.start) + 1, times.get(shift.end)):
						config.dpop.cell(row = i, column = who).fill = shift.backFillTwo
						config.dpop.cell(row = i, column = who).border = config.borderGoneTB
						
					if shift.clearance != '':
							config.dpop.cell(row = times.get(shift.start) + 2, column = who).value = shift.clearance
					assigned = True
					
			
					for i in range(config.floorStart, config.floorEnd - 1):
						if config.dpop.cell(row = times.get(shift.start) + 1, column = i).fill == shift.backFillTwo:
							tried = 0
							go = False
							break
